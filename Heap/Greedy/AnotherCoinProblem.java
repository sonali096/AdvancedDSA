...
public class AnotherCoinProblem {
    public int solve(int A) {
        // ArrayList to store powers of 5
        ArrayList<Integer> list = new ArrayList<>();
        int n = 5, ans = 1;
        list.add(ans);
        // Generate powers of 5 until it's greater than A
        while(A > ans){
            ans = ans * n;
            list.add(ans);
        }
        // Count the number of elements needed to reach A
        int count = 0;
        int i = list.size() - 1;
        // Subtract powers of 5 from A and count them
        while(A > 0){
            if(A - list.get(i) >= 0){
                count++;
                A = A - list.get(i);
